...
### 그리디 알고리즘
#### 정의
- 각 단계에서 가장 최적이라고 생각되는 선택을 하여 전체 문제를 해결하는 알고리즘
#### 기본 개념
- 탐욕적 선택 속성(Greedy Choice Property)
	- 전체적인 결과를 고려하지 않고, 각 단계에서 가장 최선의 선택을 탐욕적으로 진행하여 최적해를 찾는 방법.
- 최적 부분 구조(Optimal Substructure)
	- 전체 문제를 부분 문제로 나누어 각 부분의 최적해가 전체 문제의 최적해로 직접 연결되는 구조에 적합
- 단계적 선택
	- 전체 문제를 미리 계획하지 않고, 문제를 순차적이고 단계적으로 해결
#### 장점
- 간결성
	- 설계와 구현이 훨씬 간단
- 효율성
	- 빠르게 작동하고 메모리 사용이 많지 않음
#### 단점
- 최적해 보장 어려움
	- 각 단계에서의 최적의 선택이 전체 최적의 해결책을 보장하지 않음
- 문제의 특성에 의존
	- 특정 유형의 문제에 대해서만 적용 가능
## 13. 다이나믹 알고리즘
### 다이나믹 알고리즘
- 복잡한 문제를 더 작은 문제로 쪼개서 해결한 결과를 저장해두고, 그 결과를 반복해서 사용하는 방법
`동적 계획법 == 동적프로그래밍(DP, Dynamic Programming) == 다이나믹 알고리즘`
#### 핵심 개념
1. 문제를 작은 조각으로 나누기
	- 큰 문제를 작고 쉬운 문제로 나눈다.
2. 계산 결과를 저장해두기
	- 중복 계산을 피하기 위해, 이미 계산한 결과를 따로 저장 (Memoization)
3. 저장한 결과를 재사용하기
	- 필요한 값이 다시 나왔을 때는 저장된 결과를 불러와서 사용
#### 문제 해결에 있어 주요한 접근 방식
1. Bottom-up(Tabulation 방식)
	- 작은 문제부터 시작하여 큰 문제로 나아가면서 해결책을 구축
	- 타뷸레이션(Tabulation)방식으로 테이블에 결과를 저장
	- 구현: 반복문
2. Top-down(Memoization 방식)
	- 재귀적 방법으로 큰 문제를 작은 문제로 나누면서 해결
	- 메모이제이션(Memoization)을 사용하여 중복 계산 방지
	- 구현: 재귀 함수
##### 타뷸레이션 (Tabulation)
###### 정의
- 작은 문제의 결과부터 계산하고, 이 결과들을 누적해서 큰 문제를 해결하는 방식
- 반복문을 사용한 접근 방식
- 테이블(배열)을 사용하여 결과를 저장하고 계산
##### 메모이제이션 (Memoization)
###### 정의
- 재귀를 사용하여 하향식(Top-Down)으로 문제를 해결
- 계산된 결과를 저장하여 중복 계산을 방지하는 방식
- 재귀 호출로 스택 오버플로우 위험
- 모든 하위 문제를 미리 계산하지 않아 추가적인 저장 공간이 필요.