...
### 1의 보수 를 구하는법
- 1의 보수는 0을 1로 1을 0으로 바꾸면 쉽게 구할 수 있다.
- `~(입력받은 수)`
### 2의 보수 구하기
- 모든 자리의 수가 1인 수에 1을 더한 수를 만들어 주면 된다.
- `~(입력받은 수) + 1`
## 관계 연산자
### 관계 연산자
...
- 실수형에 대해 상등, 부등연산은 불가
	- 면접에 자주 나오는 질문 중 하나
- `>, <, <=, >=`(비교연산)
- boolean은 다른 형식으로 형변환이 불가능하다
## 논리 연산자
### 논리 연산자
- 항 혹은 연산식을 피연산자로 두는 논리합(`||`), 논리곱(`&&`) 2항 연산자
- 논리 부정 연산자(`!`)는 단항 연산자
- 값의 범위 표현 시 사용되는 것이 보통
- 결합성이 L -> R 이므로 왼쪽에서 등장하는 연산식을 우선 평가하고 이어지는 연산식을 수행할 것인지 판단
## Short circuit
### Short circuit
- 논리 식은 왼쪽부터 실행
...
- 유일한 3항 연산자
- `조건식 ? 항A : 항B`
- 논리적 오류를 피하려면 선택 대항 항은 괄호로 묶어 표기
	- `조건식 ? (항A) : (항B)`
## [필수실습 10-2] 합격/불합격 판단하기
...
## [필수실습 12-2] '\*'출력 놀이 (삼각형 기본)
## [필수실습 12-3] '\*'출력 놀이 (응용1)
## [필수실습 12-4] '\*'출력 놀이 (응용2)
## do while문
### do while
- 반복 대상 구간 코드가 조건식의 만족 여부와 관련 없이 **무조건 한 번은 실행하는 반복문**
- 기존 while과 달리 조건식 뒤에 세미콜론이 있음
## break와 continue
### break
...
- 배열의 이름은 배열 객체에 대한 참조
- Java에서 배열은 객체(클래스)로 구현되어 있음
	- length
## 문자(배)열
### 문자 배열(문자열)
- 문자열의 실체는 char[]
- 문자열 상수란 쓰기가 허용되지 않는 이름이 없는 char[]로 이해할 수 있음(통상 String 클래스로 처리)
## [필수실습 13-1] 배열에서 최댓값 찾기
## [필수실습 13-2] 버블정렬
## [필수실습 13-3] 선택정렬
## 다차원 배열
### 다차원 배열
...
## 호출 스택, 식별자 검색 순서, 정적 멤버
### 멤버 변수와 식별자 검색 순서
- 멤버 변수는 메서드처럼 클래스를 이루는 멤버
...
## [필수실습 14-1] 최댓값을 반환하는 함수
## 이벤트 루프
- `main()`함수 에서 사용자 인터페이스 출력 및 사용자 입력을 반복하는 구조
- 보통 메뉴출력과 사용자 선택을 확인
- 메뉴 선택에 따라 기능 수행
...
- Java에서 String 기본형식처럼 취급되는 경향이 있음에 유의
## 재귀호출
### 재귀 호출
...
- 함수 호출 오버헤드는 감수
- 논리 오류 발생시 Stack overflow 발생
	- 무한루프와 같은 
## [필수실습 14-2] 문자열을 뒤집어서 출력하기
## 다중정의(Overloading)
### 다중 정의(Overloading)
