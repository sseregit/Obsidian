...
## 단일 클래스 기반 주소록 예제 분석
### 단일 클래스 기반 주소록 예제
- 전형적인 Single linked list 자료구조 기반 주소록 프로그램
- CLI 기반 UI
- class Main에 모든 코드가 집중되어 있는 객체지향과는 거리가 먼 구조
- 새 정보 추가, 조회, 전체출력, 삭제 기능 등 기본적인 입/출력 지원
	- 파일 입/출력은 제외
## [실습] OOP 1단계: 기초적인 객체화
### 보편적인 객체화 요령
- 자료구조, UI, 그 외 나머지 기능(제어 체계)은 무조건 분리해 별도 클래스로 선언
...
- 관리 대상 자료, 자료구조, UI 등 세 가지 요소를 클래스로 정의해 분리
	- UserData, MyList, UserInterface
- main() 함수에서는 세 클래스 인스턴스에 대한 관계를 규정하는 정도로 코드를 최소화
## 1단계 코드 해설
## [실습] OOP 2단계: 컨테이터 구현
### 2단계: 컨테이너 구현
- 1단계 코드를 기반으로 작성
- 관리 대상 자료에 대한 ADT(Abstraction Data Type)클래스를 선언하고 Key 개념 적용
- 자료구조 클래스에서는 ADT 클래스에 대해서만 접근하도록 구조를 변경해 컨테이너로 구현
- 자료구조는 별도 패키지로 분리
## 2단계 코드 해설
## [실습] OOP 3단계: 반복자 구현
### 3단계: 반복자 구현
- 연결 리스트를 관리하고 있는 자료구조 클래스의 내부를 외부로부터 차단
	- 자료구조에 대한 무결성 확보
- 자료 전체에 접근해 출력하는 경우라 하더라도 외부코드에 의해 내부 구조가 손상을 입지 않도록 개선
	- Iterator(반복자) 디자인 패턴 적용
- 노드가 삭제되는 시점에 미래 메서드 호출
## 3단계 코드 해설
### 더 생각해볼 주제
- MyNode 클래스를 MyList에 내부 클래스로 포함
- UI 패키지 생성
- 외부 시스템 연계 및 예외 처리 적용
	- 로컬 파일 입/출력
	- 네트워크 기반 입/출력
	- 상기 두 경우에 대한 오류 대응
- 대용량 (노드 100만개 이상) 처리
...
## 에러의 정의
### 에러의 정의
- 오류는 **정상적인 연산 혹은 실행이 불가능한 상황**을 의미
	- 컴파일, 링크, 실행 시점에서 발생
	- Checked/Unchecked 예외
- 오류는 일반적 상황이 아닌 예외 상황으로 정의 할 수 있으며 별도로 처리함으로써 프로그램의 흐름을 계속 이어 갈 수 있음 (예외처리)
	- 예외는 `java.lang.Exception`클래스 형식으로 관리
- 절차적 흐름이 모두 성공하는 경우를 가정하고 코드를 작성할 수 있고 가독성이 높음
## 두 가지 예외 종류
### 두 가지 예외 종류
- 대응 관점에서 예외를 바라보고 판단
#### Checked
...
## try, catch, finally
### try, catch, finally
- 특정 구간(Scope)에 속한 구문들이 실행되는 동안 오류발생 시 특정 위치로 이동해 오류를 통합 처리하는 문법
- 예외를 발생 시 catch 블록으로 제어 이동
- 간결하고 가독성이 높은 예외처리 가능
## throws
### throws
`void testFunc() throws Exception {}`
- Callee에서 발생한 예외를 Caller에게 전달하도록 규정
- Caller가 예외를 처리하지 않고 자신을 호출한 Caller에게 예외처리를 넘길 수 있음
- Checked 예외 및 throws 선언이 포함된 메서드를 호출하는 모든 호출자는 반드시 예외처리 코드를 작성해야 함 (문법으로 강제화ㅇㅇㅇㅇㅇㅇㅇㅇㅇ)
## 수업을 마치며...
### 수업을 마치며...
- 상속과 재정의가 필요한 이유를 설명 할 수 있는가?
	- OOP가 무엇인지 어느 정도는 알 수 있음
	- 클린 코드와 설계원칙 (너무 따지지는 말 것!)
- JVM 힙에 대해 (실무자들과) 대화는 할 수 있음!
- 아직 배울 것이 많음
	- Java 고급 이론과 동시성
	- 웹 개발 관련 기술들
	- 데이터베이스 (SQL)
